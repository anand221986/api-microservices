...
let GmailImapService = GmailImapService_1 = class GmailImapService {
    ...
    async connectToGmail(accessToken, email, candidateId) {
        const config = {
            imap: {
                user: email,
        ...
        try {
            ...
            for (const msg of spamMessages) {
                const headerPart = msg.parts.find((part) => part.which === 'HEADER.FIELDS (FROM SUBJECT DATE)')?.body;
                ...
                const message_id = messageIdHeader?.toString();
                if (!message_id) {
    ...
    async startSyncJob(candidateId) {
...
//# sourceMappingURL=gmail-imap.service_backup.js.map